...
ARM_ORDER = [
    "tied_adamw_vector_adamw",
    "tied_cap_vector_adamw",
    "tied_adamw_vector_rms",
    "tied_cap_vector_rms",
    "hybrid_muon",
    "scion_c1_s1",
    "afmoun_c3_s0p5",
...
def short_arm(arm: str) -> str:
    return {
        "tied_adamw_vector_adamw": "Tied AdamW / 1D AdamW",
        "tied_cap_vector_adamw": "Tied cap / 1D AdamW",
        "tied_adamw_vector_rms": "Tied AdamW / 1D RMS",
        "tied_cap_vector_rms": "Tied cap / 1D RMS",
        "hybrid_muon": "Hybrid",
        "scion_c1_s1": "SCION",
...
def summarize_run(root: Path) -> list[dict]:
    ...
    for metrics in sorted(root.glob("*/metrics.jsonl")):
        ...
        row = {
            ...
            "factorial_modes": config.get("weight_decay_protocol", {}).get("factorial_modes"),
            "tied_mode": config.get("weight_decay_protocol", {}).get("factorial_modes", {}).get("tied"),
            "vector_mode": config.get("weight_decay_protocol", {}).get("factorial_modes", {}).get("vector"),
            "tied_wd": config.get("weight_decay_protocol", {}).get("factorial_tied_weight_decay"),
            "vector_wd": config.get("weight_decay_protocol", {}).get("factorial_vector_weight_decay"),
            "tied_cap": config.get("weight_decay_protocol", {}).get("tied_cap"),
            "tied_scale": config.get("weight_decay_protocol", {}).get("tied_scale"),
