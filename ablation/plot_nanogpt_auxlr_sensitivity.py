...
def read_rows(path: Path) -> list[dict]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            if not row.get("method") or not row.get("lr_mult"):
                continue
            try:
                row["lr_mult"] = float(row["lr_mult"])
                row["vector_lr"] = float(row["vector_lr"])
...
def tick_label(multiplier: float) -> str:
    return f"{multiplier:g}$\\times$"
...
def write_plot_summary(path: Path, rows: Sequence[dict], panels: Sequence[str]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=("panel", "method", "vector_lr", "lr_mult", "value"),
        )
        writer.writeheader()
        for panel in panels:
                ...
                writer.writerow(
                    {
                        "panel": panel,
                        "method": row["method"],
                        "vector_lr": f"{float(row['vector_lr']):.12g}",
                        "lr_mult": f"{float(row['lr_mult']):.12g}",
...
def plot(
    ...
    *,
    ...
    for method in by_method:
        by_method[method].sort(key=lambda row: row["lr_mult"])

    x_values = sorted({float(row["lr_mult"]) for row in rows})
    x_ticks = x_values
    ...
    with mpl.rc_context(paper_rcparams()):
        ...
        for panel_index, (ax, panel) in enumerate(zip(axes_1d, panels)):
            ...
            for method in ARM_ORDER:
                ...
                if not method_rows:
                    continue
                x = [float(row["lr_mult"]) for row in method_rows]
            ...
            ax.set_xlim(min(x_ticks) / 1.18, max(x_ticks) * 1.18)
