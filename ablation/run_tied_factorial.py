...
def arm_label(arm: str) -> str:
    return {
        "adamw_adamw": "tied_adamw_vector_adamw",
        "cap_adamw": "tied_cap_vector_adamw",
        "adamw_rms": "tied_adamw_vector_rms",
        "cap_rms": "tied_cap_vector_rms",
        "muon": "hybrid_muon",
        "sign": "scion_c1_s1",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Launch tied-table/1D-aux factorial ablations.")
    p.add_argument("--gpus", nargs="+", type=int, required=True)
    p.add_argument("--run-root", default="runs/ablation_tied_factorial")
    p.add_argument("--data-dir", required=True)
    p.add_argument("--seeds", nargs="+", type=int, default=[43])
    p.add_argument("--arms", default="adamw_adamw,cap_adamw,adamw_rms,cap_rms")
    p.add_argument("--sharing", choices=["full", "untied"], default="full")
    p.add_argument("--rho-output-values", nargs="+", type=float, default=None)
    p.add_argument(
        "--rho-sweep-arms",
        default="sign,afmoun",
...
def main() -> None:
    ...
    print("=== tied-table / 1D-aux factorial launcher ===", flush=True)
    ...
    worker = Path(__file__).with_name("train_tied_factorial.py")
    ...
    def launch(seed: int, arm: str, rho_output: float | None, gpu: int):
        ...
        name = f"ablation_{arm_label(arm)}_seed{seed}{suffix}_three_way_shared_vocab{sharing_suffix}"
        out_dir = root / name
        ...
        cmd = [
            ...
            "--tied-weight-decay",
            str(args.tied_weight_decay),
            "--vector-weight-decay",
            str(args.vector_weight_decay),
            "--rho-hidden",
            str(args.rho_hidden),
            "--rho-output",
            str(args.rho_output),
