...
class NanoGPT(nn.Module):
    def __init__(self, *, vocab_size: int, layers: int, heads: int, head_dim: int, mlp_hidden: int, block_size: int, scaled_relu_sq: bool):
        ...
        self.capture_tied_input_grad = False
        self._tied_input_grad_accum: torch.Tensor | None = None
    ...
    def clear_tied_input_grad_capture(self) -> None:
        self._tied_input_grad_accum = None

    def pop_tied_input_grad_capture(self) -> torch.Tensor | None:
        grad = self._tied_input_grad_accum
        self._tied_input_grad_accum = None
        return grad

    def forward(self, input_ids: torch.Tensor, labels: torch.Tensor | None = None):
        x = self.wte(input_ids)
        if self.capture_tied_input_grad and torch.is_grad_enabled():
            flat_ids = input_ids.reshape(-1).detach()

            def capture_input_grad(grad: torch.Tensor) -> torch.Tensor:
                flat_grad = grad.reshape(-1, grad.shape[-1]).detach().float()
                if self._tied_input_grad_accum is None:
                    self._tied_input_grad_accum = torch.zeros_like(
                        self.wte.weight, dtype=torch.float32
                    )
                self._tied_input_grad_accum.index_add_(0, flat_ids, flat_grad)
                return grad

            x.register_hook(capture_input_grad)
...
def tensor_rms(t: torch.Tensor) -> float:
    tf = t.detach().float()
    return float(tf.square().mean().sqrt().item())
...
def tied_source_grad_stats(
    total_grad: torch.Tensor | None,
    input_grad: torch.Tensor | None,
) -> dict:
    if total_grad is None or input_grad is None:
        return {}
    total = total_grad.detach().float()
    inp = input_grad.detach().float()
    out = total - inp
    eps = 1e-30
    input_row_norm = inp.norm(dim=1)
    output_row_norm = out.norm(dim=1)
    total_row_norm = total.norm(dim=1)
    both_rows = (input_row_norm > 0) & (output_row_norm > 0)
    total_norm = total.norm()
    input_norm = inp.norm()
    output_norm = out.norm()
    return {
        "tied_input_nonzero_rows_frac": float((input_row_norm > 0).float().mean().item()),
        "tied_output_nonzero_rows_frac": float((output_row_norm > 0).float().mean().item()),
        "tied_total_nonzero_rows_frac": float((total_row_norm > 0).float().mean().item()),
        "tied_rows_hit_both_frac": float(both_rows.float().mean().item()),
        "tied_input_grad_rms": tensor_rms(inp),
        "tied_output_grad_rms": tensor_rms(out),
        "tied_total_grad_rms": tensor_rms(total),
        "tied_input_grad_norm": float(input_norm.item()),
        "tied_output_grad_norm": float(output_norm.item()),
        "tied_total_grad_norm": float(total_norm.item()),
        "tied_input_norm_frac": float(input_norm.div(total_norm + eps).item()),
        "tied_output_norm_frac": float(output_norm.div(total_norm + eps).item()),
        "tied_input_output_cos": cosine(inp, out),
        "tied_input_total_cos": cosine(inp, total),
        "tied_output_total_cos": cosine(out, total),
    }


def apply_tied_grad_source(
    model: NanoGPT,
    source: str,
    input_grad: torch.Tensor | None,
) -> dict:
    tied = model.get_input_embeddings().weight
    total_grad = tied.grad
    stats = tied_source_grad_stats(total_grad, input_grad)
    stats["tied_grad_source_applied"] = source
    if source == "both":
        return stats
    if total_grad is None or input_grad is None:
        raise RuntimeError("missing tied-table gradient split")
    if source == "input_only":
        tied.grad = input_grad.to(dtype=total_grad.dtype)
    elif source == "output_only":
        tied.grad = (total_grad.detach().float() - input_grad.detach().float()).to(
            dtype=total_grad.dtype
        )
    else:
        raise ValueError(f"unknown tied gradient source: {source}")
    return stats
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--tied-grad-source",
        choices=["both", "input_only", "output_only"],
        default="both",
        help="Which tied-table gradient source to use for the AF update.",
    )
    ...
    p.add_argument("--skip-final-checkpoint", action="store_true", help="Do not save the default final latest_full.pt checkpoint.")
    return p.parse_args()
...
def main() -> None:
    ...
    if args.check_config:
        ...
        print(json.dumps({
            ...
            * (
            ...
            "tied_grad_source": args.tied_grad_source,
        }, indent=2))
        return
    ...
    model.train()
    model.capture_tied_input_grad = args.arm == "afmoun"
    ...
    while step < int(args.iterations):
        ...
        model.clear_tied_input_grad_capture()
        ...
        tied_source_stats = apply_tied_grad_source(
            model,
            args.tied_grad_source,
            model.pop_tied_input_grad_capture(),
        ) if args.arm == "afmoun" else {}
        grad_norm = None
        if scaler is not None:
            scaler.unscale_(opt)
        ...
        if step == 1 or step % int(args.diag_every_steps) == 0:
            write_jsonl(metrics, {"phase": "diagnostic", "step": step, "tokens_seen": tokens_seen, **weight_stats(model), **tied_source_stats, "optimizer_state_dtypes": optimizer_state_dtypes(opt), "seconds": seconds})
    ...
    if not args.benchmark_memory and not args.skip_final_checkpoint:
        path = ck_dir / "latest_full.pt"
        torch.save({"model_state_dict": model.state_dict(), "optimizer_state_dict": opt.state_dict(), "step": step, "tokens_seen": tokens_seen, "config": config_payload}, path)
        write_jsonl(metrics, {"phase": "checkpoint", "kind": "full_latest", "step": step, "tokens_seen": tokens_seen, "path": str(path), "seconds": time.time() - started})
