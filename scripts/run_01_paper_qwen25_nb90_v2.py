...
def validate_data_cache(data_dir: Path, requested_train_tokens: int, requested_eval_tokens: int, block_size: int) -> None:
    ...
    if usable_train < requested_train_tokens:
        raise RuntimeError(f"Qwen cache too small: usable_train={usable_train:,} requested={requested_train_tokens:,}")
    if usable_eval < requested_eval_tokens:
        raise RuntimeError(f"Qwen eval cache too small: usable_eval={usable_eval:,} requested={requested_eval_tokens:,}")
    print(
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run paper Qwen2.5-0.5B NB90-compatible V2 jobs.")
    p.add_argument("--gpus", nargs="+", type=int, default=[5, 6, 7])
    p.add_argument("--run-root", default="runs/01_paper_main_results_qwen25_0p5b_nb90_v2")
    ...
    p.add_argument("--resume-existing-full", action="store_true")
...
def build_job_command(args: argparse.Namespace, arm: str, seed: int, out_dir: Path) -> list[str]:
    cmd = [
        worker_python(),
        str(ROOT / "scripts" / "train_01_paper_qwen25_nb90_v2.py"),
        "--model-dir",
        ...
        "--full-checkpoint-every-steps",
        str(args.full_checkpoint_every_steps),
        "--seed",
        str(seed),
        "--bf16",
    ]
    ...
    if args.resume_existing_full:
        resume_path = out_dir / "checkpoints" / "latest_full.pt"
...
def make_jobs(args: argparse.Namespace) -> list[dict]:
    ...
    for seed in seeds:
        for arm in arms:
            name = f"qwen25_0p5b_{arm_label(arm)}_seed{seed}_nb90_v2_1b"
            out_dir = Path(args.run_root) / name
            done = (
                (out_dir / "metrics.jsonl").exists()
                and not args.retry_incomplete
                and not args.resume_existing_full
            )
...
def main() -> None:
    ...
    print("=== 01 paper Qwen2.5-0.5B NB90-compatible V2 launcher ===", flush=True)
    ...
    print(
        ...
        f"checkpoint_every={args.checkpoint_every_steps} full_latest_every={args.full_checkpoint_every_steps}",
        flush=True,
    )
    ...
    if failures:
        raise RuntimeError(f"paper Qwen jobs failed: {failures}")
    print("All paper Qwen NB90-compatible V2 jobs completed.", flush=True)
