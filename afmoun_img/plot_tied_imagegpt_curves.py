"""Paper-style curves for tied ImageGPT-style AF-Muon experiments.

This mirrors the language-model optimizer-curve template, but uses image-facing
metrics: training loss, validation loss, and validation bits/dim.
"""
...
PANEL_SPECS = {
    ...
    "eval_bits_per_dim": ("eval", "eval_bits_per_dim", "Validation bits/dim"),
    "eval_ppl": ("eval", "eval_ppl", "Validation perplexity"),
}
DEFAULT_PANELS = ("train_loss", "eval_loss", "eval_bits_per_dim")
...
def infer_arm(run_name: str) -> str:
    ...
    if "afmoun" in parts or "af_muon" in normalized or ("af" in parts and "c3" in parts):
        return "AF-Muon"
...
def read_metrics(
    ...
    *,
    ...
    plot_min_step: int = 0,
    train_step_stride: int = 0,
    eval_step_stride: int = 0,
) -> list[dict]:
    rows: list[dict] = []
    ...
    for metrics_path in sorted(run_root.glob("*/metrics.jsonl")):
        ...
        if arm == "unknown":
            ...
            continue
        if seed is None:
            raise ValueError(f"could not infer seed from run directory: {run_name}")
        ...
        with metrics_path.open("r", encoding="utf-8") as f:
            for line_number, line in enumerate(f, start=1):
                ...
                try:
                    ...
                    step = 0
                if step < int(plot_min_step):
                    continue
                if phase == "train" and train_step_stride > 0 and step % int(train_step_stride) != 0:
                    continue
                if phase == "eval" and eval_step_stride > 0 and step % int(eval_step_stride) != 0:
                    continue
                row["arm"] = arm
                row["seed"] = seed
    ...
    duplicates = {k: v for k, v in runs_by_arm_seed.items() if len(v) > 1}
    if duplicates:
        details = "; ".join(
            f"{arm}, seed {seed}: {sorted(names)}"
            for (arm, seed), names in sorted(duplicates.items())
        )
        raise ValueError(f"multiple run directories share an optimizer/seed: {details}")
    return rows
...
def aggregate(
    ...
    *,
    ...
    for arm, seed_points in by_seed.items():
        grids = [frozenset(points) for points in seed_points.values()]
        if len(set(grids)) > 1:
            warnings.warn(
                f"{arm} has unequal {phase}/{value_key} checkpoint grids across seeds; "
                "each token is summarized over available seeds.",
                stacklevel=2,
            )
        ...
        for token in all_tokens:
            ...
            sample_sds.append(float(np.std(values, ddof=1)) if values.size >= 2 else math.nan)
...
def token_scale(rows: Sequence[dict], requested: str) -> tuple[float, str]:
    if requested == "billions":
        return 1e9, "Training tokens (billions)"
    if requested == "millions":
        return 1e6, "Training tokens (millions)"
    maximum = max((float(r.get("tokens_seen", 0)) for r in rows), default=0.0)
    if maximum >= 1e9:
        return 1e9, "Training tokens (billions)"
    return 1e6, "Training tokens (millions)"


def nice_axis_max(rows: Sequence[dict], divisor: float) -> float | None:
    tokens = []
    for row in rows:
        try:
...
def uncertainty_half_width(sd: np.ndarray, count: np.ndarray, uncertainty: str) -> np.ndarray:
    if uncertainty == "none":
...
def marker_indices(n_points: int, maximum_markers: int = 8) -> list[int]:
    if n_points <= 0:
        return []
...
def plot_curves(
    ...
    *,
    ...
    divisor, xlabel = token_scale(rows, token_unit)
    x_axis_max = nice_axis_max(rows, divisor)
    ...
    with mpl.rc_context(paper_rcparams()):
        ...
        for panel_index, (ax, panel) in enumerate(zip(axes_1d, panels)):
            ...
            for arm in ARM_ORDER:
                ...
                x = stats["tokens"].astype(np.float64) / divisor
                mean = stats["mean"]
                half_width = uncertainty_half_width(stats["sd"], stats["count"], uncertainty)
            ...
            ax.set_title(f"({chr(ord('a') + panel_index)}) {panel_title}", loc="left", pad=3.0)
            ax.set_ylabel("")
            if x_axis_max is not None:
                right_pad = 0.03 * x_axis_max if divisor >= 1e9 else 0.015 * x_axis_max
                ax.set_xlim(left=0 if x_min is None else x_min, right=x_axis_max + right_pad)
            else:
                ax.set_xlim(left=0 if x_min is None else x_min)
            ax.margins(x=0)
            if divisor >= 1e9 and x_axis_max is not None and x_axis_max <= 5.0:
        ...
        fig.subplots_adjust(left=0.060, right=0.995, bottom=0.235, top=0.77, wspace=0.30)
        fig.supxlabel(xlabel, x=0.54, y=0.045, fontsize=8.0)
...
def parse_formats(items: Iterable[str]) -> tuple[str, ...]:
    formats: list[str] = []
    for item in items:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create paper-style curves for tied ImageGPT-style runs."
    )
    ...
    parser.add_argument("--uncertainty", choices=("sd", "sem", "none"), default="sem")
    parser.add_argument("--token-unit", choices=("auto", "millions", "billions"), default="billions")
    parser.add_argument("--plot-min-tokens", type=int, default=0)
    parser.add_argument("--plot-min-step", type=int, default=0)
    parser.add_argument(
        "--train-step-stride",
        type=int,
        default=0,
        help="Keep only train rows whose step is divisible by this value; 0 keeps all train rows.",
    )
    parser.add_argument(
        "--eval-step-stride",
        type=int,
        default=0,
        help="Keep only eval rows whose step is divisible by this value; 0 keeps all eval rows.",
    )
    ...
    parser.add_argument("--strict-arms", action="store_true")
    ...
    rows = read_metrics(
        ...
        plot_min_step=int(args.plot_min_step),
        train_step_stride=int(args.train_step_stride),
        eval_step_stride=int(args.eval_step_stride),
    )
    if not rows:
    ...
    summary_path = args.summary_csv or output_stem.with_name(f"{output_stem.name}_summary.csv")
