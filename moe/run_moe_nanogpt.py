...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Launch tied-embedding NanoGPT-MoE jobs.")
    p.add_argument("--gpus", nargs="+", type=int, required=True)
    p.add_argument("--data-dir", required=True)
    p.add_argument("--run-root", default="runs/moe_nanogpt_50k_top1_e4_routeraux0p01_3seeds_2p67b")
...
def command(args: argparse.Namespace, arm: str, seed: int, out_dir: Path) -> list[str]:
    ...
    cmd = [
        worker_python(),
        str(Path(__file__).with_name("train_moe_nanogpt.py")),
        ...
        "--router-z-loss-coef", str(args.router_z_loss_coef),
        "--router-aux-loss-coef", str(args.router_aux_loss_coef),
...
def main() -> None:
    ...
    for seed in seeds:
        for arm in arms:
            name = f"moe_nanogpt_top{args.top_k}_e{args.num_experts}_{arm_label(arm)}_seed{seed}"
            jobs.append({"seed": seed, "arm": arm, "name": name, "out_dir": Path(args.run_root) / name})

    print("=== Tied-embedding NanoGPT-MoE launcher ===", flush=True)
    ...
    if failures:
        raise RuntimeError(f"NanoGPT-MoE jobs failed: {failures}")
