...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Launch NanoGPT AF-Muon jobs.")
    p.add_argument("--gpus", nargs="+", type=int, required=True)
    p.add_argument("--data-dir", required=True)
    p.add_argument("--run-root", default="runs/nanogpt_scion_recipe")
...
def arm_label(arm: str) -> str:
    return {
        ...
        "fact_adamw_adamw": "factorial_tied_adamw_1d_adamw",
        "fact_c3_adamw": "factorial_tied_c3_1d_adamw",
        "fact_adamw_rms": "factorial_tied_adamw_1d_rms",
        "fact_c3_rms": "factorial_tied_c3_1d_rms",
    }[arm]


def aux_weight_decay_for_arm(args: argparse.Namespace, arm: str) -> float:
    if arm == "muon" and args.hybrid_aux_weight_decay is not None:
        return float(args.hybrid_aux_weight_decay)
    if arm == "sign" and args.sign_aux_weight_decay is not None:
        return float(args.sign_aux_weight_decay)
    if arm == "afmoun" and args.afmoun_aux_weight_decay is not None:
        return float(args.afmoun_aux_weight_decay)
    return float(args.aux_weight_decay)


def command(args: argparse.Namespace, arm: str, seed: int, out_dir: Path) -> list[str]:
    worker = Path(__file__).with_name("train_nanogpt.py")
    cmd = [
        worker_python(),
        str(worker),
        ...
        "--muon-lr-multiplier", str(args.muon_lr_multiplier),
        "--multiplier-mode", args.multiplier_mode,
        "--vector-lr", str(args.vector_lr),
        "--momentum", str(args.momentum),
        "--matrix-weight-decay", str(args.matrix_weight_decay),
        "--aux-weight-decay", str(aux_weight_decay_for_arm(args, arm)),
        ...
        "--warmdown-frac", str(args.warmdown_frac),
        "--activation-mode", args.activation_mode,
...
def main() -> None:
    ...
    for seed in seeds:
        for arm in arms:
            name = f"nanogpt_{arm_label(arm)}_seed{seed}_scion_recipe"
            jobs.append({"seed": seed, "arm": arm, "name": name, "out_dir": Path(args.run_root) / name})

    print("=== NanoGPT AF-Muon launcher ===", flush=True)
    ...
    if failures:
        raise RuntimeError(f"NanoGPT jobs failed: {failures}")
